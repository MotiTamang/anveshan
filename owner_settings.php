<?php
// owner_settings.php
// Settings page for owners - change password, delete account
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    ...
    if (empty($new_name) || empty($new_email) || empty($new_phone)) {
        ...
        $check_stmt->bind_param('si', $new_email, $owner_id);
        $check_stmt->execute();
        if ($check_stmt->get_result()->num_rows > 0) {
            $error = 'Email is already in use by another account.';
        } else {
            $update_stmt = $conn->prepare("UPDATE users SET name = ?, email = ?, phone = ? WHERE user_id = ?");
            $update_stmt->bind_param('sssi', $new_name, $new_email, $new_phone, $owner_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'change_password') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        if ($user_data && password_verify($current_password, $user_data['password'])) {
            ...
            $update_stmt->bind_param('si', $hashed, $owner_id);
            if ($update_stmt->execute()) {
...
$fetch_stmt->bind_param('i', $owner_id);
$fetch_stmt->execute();
...
<section class="seeker-dashboard-section">
    <div class="seeker-wrap">
        <div class="top-row">
            <h1>Settings</h1>
            <div class="actions">
                <a class="btn" href="owner_profile.php">My Profile</a>
        ...
        <div class="panel">
            ...
            <form method="post" action="owner_settings.php" style="max-width: 500px;">
        ...
        <div class="panel" style="border: 2px solid #dc3545;">
            ...
            <form method="post" action="owner_settings.php" onsubmit="return confirm('Are you absolutely sure you want to delete your account? This action cannot be undone!');" style="max-width: 500px;">
